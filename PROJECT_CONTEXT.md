...
- 2026-07-18 — **Infra incident: `trw2-london` session (nominal start Thu 2026-07-16 08:30 UK) ran 2+ real days in wall-clock time**, not finishing until Sat 2026-07-18 ~15:47 UTC. Worse recurrence of the single-day anomaly logged 2026-07-16 (see `journal/results_log.jsonl` sig-087 entry / that day's now-superseded handoff). New evidence this time: `chart_get_state`/`tv_health_check` repeatedly returned symbols queried earlier in the *same* session well after moving on, and near the end showed `BINANCE:BTCUSDT` — a symbol never requested by this session — strongly suggesting **a concurrent scheduled session (the Sat 14:00 UK `trw2-crypto-sat-mid` routine) shared and overwrote the same TradingView CDP chart state mid-analysis**. Net effect: a XAUUSD Setup B candidate (entry 4013-4021, SL 4044, TP1 3975/TP2 3968, confidence 82) was correctly identified but went stale (price wicked through the zone at bar 1784214900, high 4017.03) before the session could publish it — correctly withheld per §10, never logged to `signals_log.jsonl`/`active_setups.json`. No trading-state files were corrupted: `trw2-crypto-sat-mid` ran cleanly and its `handoff_latest.md`/`active_setups.json` (sig-089 BTCUSDT) were left intact and are the current source of truth, not overwritten by this note. Flagging for Magic: worth checking whether scheduled tasks can run concurrently against the same TradingView CDP target/window, and whether that should be prevented (separate tabs, a lock file, or serialized scheduling).
