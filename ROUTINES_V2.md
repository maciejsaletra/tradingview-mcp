...
**Routing per typ setupu (2026-07-18):** powyższa mapa instrumentów dotyczy WYŁĄCZNIE setupów day trading (TOP 3/XAU intraday) i scalpów. **Każdy setup z `lifecycle: "swing"` (§7c/§7d) idzie WYŁĄCZNIE do tematu `swing` — "SWINGI H4/D1"** (`message_thread_id: 9`, chat `-1003969670552`, `.env: TELEGRAM_THREAD_SWING=9`) — nigdy do tematu instrumentu. Przed każdą wysyłką sprawdź lifecycle/strategy_type setupu: swing → `--topic swing`; **scalp XAU (`strategy_type: "scalp_xau"`) → WYŁĄCZNIE `--topic scalp_xau` — temat "SCALPING XAU" (`message_thread_id: 2851`, `.env: TELEGRAM_THREAD_SCALP_XAU=2851`), nigdy temat `xau`**; intraday day trading → temat instrumentu z mapy.
...
2. **Carry-over status check (mandatory, every run, no exceptions).** **Parallelization note (2026-07-22): this step's `data_get_ohlcv` bar-walk calls (CDP) and step 5's Krok 0 macro review (WebSearch/WebFetch/`calendar` CLI, zero CDP) may be issued in the same tool-call turn/message — different backends, no shared state. This does NOT extend to any two steps that both use `chart_set_symbol`/CDP — those must stay sequential, there is exactly one active chart context for the whole process (`src/connection.js`, single CDP client/target). Do not attempt parallel `chart_set_symbol` calls across different instruments — this will corrupt whichever call loses the race.** for every entry already in `memory/active_setups.json`, `chart_set_symbol` and check status vs entry/SL/TP1/TP2/TP3. **Both states require an intrabar bar-walk, not just a live quote** — pull `data_get_ohlcv` covering the period since `last_checked_at` (M5 for tight levels, M15 otherwise; if the gap since `last_checked_at` exceeds ~40h of traded time, use M15 so the 500-bar cap still covers the whole window) and walk the bars chronologically:
   - **Not-yet-triggered setup — sprawdzaj RÓWNOLEGLE trzy warunki (2026-07-19):**
     - **(a) Touch entry:** any bar touched the entry zone → setup activated between runs; continue the walk from that bar for SL/TP touches exactly like a triggered setup.
     - **(b) Gap past zone:** price has gapped *past* the zone without touching it → mark `missed`/`expired`.
     - **(c) TP1 pre-trigger:** cena osiągnęła TP1 (lub dalej — TP2/TP3) PRZED dotknięciem entry zone → **status = `missed` (nie pending)**. Nie czekaj do naturalnego expiry — oznacz natychmiast przy najbliższym bar-walku, który to wykryje. Journal entry (results_log, verbatim format): `"TP[n] pre-trigger @ [timestamp baru przecięcia] — setup missed, no position taken"`, `rr_realized: 0`. Jeśli (c) prawdziwe → ma **priorytet nad kontynuacją pending**. Dotyczy wszystkich strumieni (day trading, crypto weekend, scalp XAU); dla crypto weekend/scalp XAU jest to jednocześnie §9b Trigger A pkt 3 → w tym samym runie świeży scan + nowa karta.
     - If none of (a)/(b)/(c) fired, a fresh `quote_get` completes the status — setup pozostaje pending. A live quote back outside the zone looks identical to "never entered"; only the bar-walk distinguishes them (dlatego (a)/(b)/(c) zawsze z bar-walku, nigdy z samego quote).
...
5. **Krok 0 — Makro/news/DXY/VIX review** (§6) — execute fully before any chart analysis. (May run concurrently with step 2's bar-walk — see parallelization note under step 2.)
...
6.5. **M5 Scalp XAU "na dzień dobry" [optional; od 2026-07-18 wykonywany przez RÓWNOLEGŁE rutyny `trw2-scalp-xau-asia/london/ny` (§2), nie jako krok rutyn sesyjnych]** — instrument: **WYŁĄCZNIE XAUUSD** (żadnej watchlisty, żadnych innych instrumentów). Każda rutyna scalp wykonuje 4 przebiegi co 30 min w swoim oknie 2h; każdy przebieg uruchamia ten blok tylko gdy WSZYSTKIE prerekwizyty P1–P5 przechodzą; inaczej skip silently (no journal entry needed for a clean skip). Rutyny sesyjne (`trw2-asia/london/newyork`) NIE wykonują już tego bloku.
   **Wyłączenie z gwarancji/degradacji (2026-07-18):** moduł scalp "na dzień dobry" jest WYŁĄCZONY z mechanizmu gwarancji i degradacji confidence (§7 krok 8c Krok 5 / §1.9) — pozostaje w pełni warunkowy: [optional, session-open only], próg confidence **60** (ujednolicony z resztą systemu, obniżony z 65 dnia 2026-07-18), poniżej → STOP bez drabinki degradacji, otherwise skip silently. Gwarancja liczby setupów (zawsze 3 TOP 3 + 1 XAU) dotyczy wyłącznie tych dwóch kategorii — scalp jest bonusem ponad tę gwarancję, nigdy jej częścią.
   ...
   **Prerequisites:**
   ...
   - **P5 — slot free:** no entry in `memory/active_setups.json` with `strategy_type: "scalp_xau"` from this same session window.

   **Execution sequence (only if P1–P5 all pass; każdy z 4 przebiegów okna):**
     0. **Quick-reject pre-check (dodane 2026-07-23, uproszczony wariant §7 krok 8 podkrok 0 dla scalpa):** jeden tani `data_get_ohlcv(summary:true)` na oknie referencyjnym z kroku 1 (Asian/London/NY H/L range). Jeśli `|change_pct| < 0.15%` ORAZ `(high−low)/close < 0.3%` dla tego okna → STOP silently, skip jak standardowy clean skip (bez wpisu do journala). Inaczej kontynuuj krok 1 normalnie. Cel: unikaj pełnej sekwencji P1–P5 + kroki 1–13 na przebiegu, gdzie referencyjny zakres jest jednoznacznie martwy (płaski, wąski) — ten sam próg co day trading, nie rozluźniać bez danych.
     ...
     7. Compute ENTRY — przecięcie strefy OB/FVG M5 z zakresem **Fibo 0.5–0.7 (Golden Pocket jako przedział, nie pojedynczy poziom 0.618; zmiana 2026-07-18)**, limit at zone mid, confirmed by M3 trigger; priorytet: FVG pokrywające się z OB na wyższej ramie (bez zmian). / SL (beyond sweep extreme + 0.1% buffer) / TP1 (nearest M5 imbalance, min **1.5R** — obniżone z 2.0R, 2026-07-18) / TP2 (session reference high/low, min 2.5R — bez zmian). If TP1 < 1.5R → stop, niezależnie od confidence.
     ...
     13. Send via Bash CLI → **WYŁĄCZNIE temat "SCALPING XAU"** (`--topic scalp_xau`, `message_thread_id: 2851`, chat `-1003969670552`, `.env: TELEGRAM_THREAD_SCALP_XAU=2851`) — NIGDY do tematu `xau` ani żadnego innego, mimo że instrument to XAU. Caption must include the four v2 fields + tag `"⚡ SCALP XAU · zamknięcie do [HH:MM] UK"`. **Superseded by the text-first pilot below (13a-13e) — step 13's send-doc-only behavior is replaced by that sequence for scalp_xau specifically; steps 1-12 above are unchanged.**

     **TEXT-FIRST PILOT — scalp XAU only (2026-07-22).** Replaces step 13's send order. Does not change any quality gate (P1-P5, E7, confidence scoring, RR≥1.5 all unchanged) — only when/how the result gets sent.
     - **13a.** Immediately after step 11 (append to `journal/signals_log.jsonl`, now with `decision_at` = the moment step 9's Signal Risk Engine check completed — before the screenshot/card pipeline) — sanity-check Entry/SL/TP1/confidence are non-null (they're already computed by steps 7-9; this is just a guard before sending) → `node src/cli/index.js telegram send-text --topic scalp_xau --text "<manually MarkdownV2-escaped>"` with direction, entry, SL, TP1, confidence, `session_close_by`. **Escaping warning: `send-text --text` does NOT auto-escape MarkdownV2 reserved characters (`. - ( ) !` etc.) — escape them manually or Telegram rejects the message** (`send-doc --caption` escapes automatically, `send-text` does not — see `memory/project_telegram_markdownv2_escaping.md`). Set `text_sent_at` on the same `signals_log.jsonl` entry (update the line, not a new one).
     - **13b.** Step 12 (`active_setups.json` write) proceeds after 13a, unchanged.
     - **13c.** Card pipeline (step 10's screenshot → `tv ohlcv` → `tv chart-render` → fill → `tv card render`) — invoke the render with Bash `timeout: 300000` (5 min). Immediately before rendering: fresh `quote_get`, compute `|current_price − entry_mid| / |tp1 − entry_mid|` relative to `decision_at`; if `>= 0.40` → append `" · ⚠️ rynek już się poruszył od sygnału"` to the card's `<SUBTITLE>` field when filling the template (no new placeholder — `context/commercial_card_template.html` stays untouched) and add the same sentence to the 13e-style follow-up text if applicable.
     - **13d.** Render succeeds → `telegram send-doc` with the PNG exactly as step 13 describes, set `card_sent_at`.
     - **13e.** Render times out (Bash returns after 300000ms) or errors → do NOT retry. Record `card_status: "render_failed"` on the `signals_log.jsonl` entry, send `telegram send-text --topic scalp_xau --text "⚠️ Karta niedostępna dla [ID] - zobacz wcześniejszy tekst dla poziomów"` (escaped), leave `card_sent_at` as `null`.
...
7. **XAU STREAM (zawsze pierwszy, osobny od reszty watchlisty):**
   ...
   c. Metoda 2: confluence multi-element — `data_get_pine_boxes` (liquidity zones), Fib levels (0.618 entry / 1.272 TP1 / 1.618 TP2), DXY correlation (from Krok 0), BOS/CHoCH direction. **`data_get_ohlcv` (dodane 2026-07-22): NIE pobieraj ponownie — współdziel bary H1/M15 już pobrane w Metodzie 1 dla tego samego (symbol, timeframe) w tym przebiegu; drugie pobranie tej samej pary jest zakazane.**
...
8. **TOP 3 WATCHLIST (bez złota):**
   ...
   0. **Quick-reject pre-scan (dodane 2026-07-22):** przed pełnym deep-dive, pobierz jeden tani `data_get_ohlcv(summary:true)` per instrument z tej watchlisty (10 wywołań, bez pine boxes/lines/labels, bez zmiany timeframe). **Odrzuć z pełnej analizy TYLKO gdy OBA warunki prawdziwe** (celowo wąski próg — odsiewa wyłącznie jednoznacznie martwe instrumenty, nie ryzykuje pominięcia realnego range-fade setupu): `|change_pct| < 0.15%` ORAZ `(high−low)/close < 0.3%`. Instrument spełniający oba → pomiń dla niego H1/M15/M5 deep-dive i pine boxes/lines/labels w tym przebiegu, zanotuj w handoff jedną linią: `"quick-reject: [SYMBOL] (change X%, range Y%)"`. Wszystkie pozostałe (w tym instrumenty tylko "trendujące bez wielkiej zmienności") dostają pełną analizę jak dotąd — próg do rozluźnienia dopiero po realnych danych z §7 krok 17b, nie zgadywać agresywniej teraz.
   a. Dla każdego (który przeszedł quick-reject): przelicz confidence Metody 1 (§8) + Freshness Check 1 (§10).
   b. Jeśli setup ma confluence Metody 2 (liquidity zone, Fib, pine boxes) → oznacz „confluence boost" (+5 pts per §7b E3). **OHLCV reuse (2026-07-22):** Metoda 2 współdzieli bary H1/M15 już pobrane w Metodzie 1 (krok a) dla tego samego instrumentu — nie pobieraj drugi raz.
...
14. **⚠️ SEND-ORDER GATE (added 2026-07-17 after 2 consecutive violations — trw2-newyork and trw2-ny2 both sent carry-over before pre-session):** before sending ANY Telegram card this step, answer explicitly: "Has the pre-session analysis card (14a) already been sent this run?" If the answer is no, send 14a NOW, before touching 14b/14c, even if the carry-over/signal card content was prepared first chronologically. Preparing a card's content out of order is fine; **sending** it out of order is not. Send to Telegram via CLI (never the MCP tool — see `context/telegram_config_notes.md`), in this fixed order:
    a. **Pre-session analysis card (mandatory, every session)** — the approved rich analysis layout at `context/session_analysis_template.html` (approved 2026-07-06 from reference screenshot; locked, don't redesign): header with ticker/price/bias badge, previous-setup status, market context, D1/H4/phase/probability tiles, session plan, scenarios A/B/C with %, liquidity map (BSL/SSL), active FVGs, fibonacci, transaction levels with hit-checkmarks, RR tiles. Weekdays: **XAUUSD, always** → `xau` topic. Weekend crypto sessions: BTCUSDT as primary → `krypto` topic — **wariant weekendowy (2026-07-18): kafelki D1/H4 z szablonu ZASTĄP kafelkami H1/M15 (zakaz D1/H4 dla crypto, §4), sekcje TOP 3/XAU nie występują — zamiast nich BTC (wymagany, gwarancja §9) + watchlista crypto (0–2 wiersze)**. Render `card render --width 1100`. The card's "POPRZEDNI SETUP" section gives a short summary of the primary instrument's carry-over — it does NOT replace the primary instrument's own carry-over status card (step 2 applies to EVERY active setup, primary included).
    ...
    c. New signal cards — **WYŁĄCZNIE commercial template (`context/commercial_card_template.html`) z osadzonym PRAWDZIWYM wykresem (2026-07-20, obowiązkowe):** pipeline `tv ohlcv --count 120` (zapis bars do JSON przez Bash, nie PowerShell pipe — BOM) → `tv chart-render --data <bars.json> --entry-low --entry-high --sl --tp1 --tp2 --symbol <SYM> --tf 15 --out <png>` → osadź jako `<img class="chart-img">`. **Szablon `context/signal_card_template_v2.html` (syntetyczny canvas, proceduralne świece) jest WYCOFANY dla kart publikowanych** — syntetyczny wykres tworzy ryzyko wizerunkowe (subskrybent porówna kartę z własnym terminalem i zobaczy inne świece), nawet gdy poziomy TP/SL/entry są poprawne. Zakaz publikacji karty sygnałowej bez realnego wykresu. **PRE-SEND PLACEHOLDER CHECK (2026-07-20, incydent sig-101–104):** ostatni krok przed KAŻDĄ wysyłką karty na Telegram = grep wynikowego HTML za wzorcem `<[A-Z_0-9]{2,}>` — jakikolwiek wynik → STOP, nie wysyłaj, napraw fill. To sprawdzenie jest NIEZALEŻNE od walidatora wbudowanego w `card render` (który failuje na niewypełnionych placeholderach; `--force` tylko do testów wewnętrznych, nigdy do publikacji) — defense in depth: dwa niezależne sprawdzenia, nie jedno. Dodatkowo sanity: PNG karty < 100 KB = czerwona flaga (wypełnione karty mają ~150–250 KB). (Regresja wykryta 2026-07-20: trw2-asia wygenerowała sig-092–096 szablonem v2 wbrew tej regule; sig-091 BTC poszedł poprawnie.) Each to its instrument topic, **EXCEPT lifecycle:"swing" → always `--topic swing` ("SWINGI H4/D1", thread 9), never the instrument topic (routing rule §3, 2026-07-18)**; scalp_xau → `--topic scalp_xau`. **Card generation canon v2 (2026-07-09):** before generating any new signal card, read `context/card_generation_prompt_v2.md` — it defines the mandatory card structure: MAPA PŁYNNOŚCI → 🔄 PULLBACK/OTE (M15 impulse, 0.618–0.786, rozszerzone 0.5 przy wysokim wolumenie) → 🔁 PULLBACK MOŻLIWY (mandatory, at end) → PLAN SESYJNY (UK time, Azja 00-07/Londyn 08-12/NY 13-21) → SCENARIUSZE → FIBONACCI/POZIOMY. Key rules (aligned 2026-07-18): D1/H4 KATEGORYCZNIE ZAKAZANE na kartach day tradingu (zero odniesień — tylko karty swing używają D1/H4); Confluence Zone module (≥2 TF overlap, day trading = tylko H1/M30/M15); HTF POI module wyłącznie karty swing (day trading: POI H1); M5 trigger override allowed for confluence zone or high volatility (not just lifecycle:"scalp"); all output times in Europe/London.
14d. **TEXT-FIRST FLOW (2026-07-23, rozszerzone z pilota scalp-xau §7 krok 6.5 kroki 13a–13e na WSZYSTKIE 12 rutyn publikujących: 4 sesyjne + 5× crypto weekend + 3× scalp-xau — nie daily-close/weekly-review, które nie publikują nowych sygnałów).** Zastępuje kolejność wysyłki dla kroku 14c (nowe sygnały) — 14a (pre-session, SEND-ORDER GATE) i 14b (carry-over) bez zmian, mają pierwszeństwo jak dotąd. Nie zmienia żadnej bramki jakości (Signal Risk Engine, Freshness Check 1/2, filtr 5b-1, confidence, RR) — wyłącznie kolejność/timing wysyłki:

  a. Natychmiast po kroku 11 (zapis do `signals_log.jsonl`, z polem `decision_at` = moment zakończenia kroku 11 Signal Risk Engine — przed pipeline'em screenshot/karta): sanity-check że Entry/SL/TP1/confidence są non-null (już policzone w krokach 7–10, to tylko guard przed wysyłką) → `telegram send-text` na właściwy temat (instrument/`swing`/`scalp_xau` wg §3 routingu) z kierunkiem, entry, SL, TP1, confidence, `reason_short` skrócony. **Ręczne escapowanie MarkdownV2** (`send-text` nie escapuje automatycznie, w przeciwieństwie do `send-doc --caption` — patrz `memory/project_telegram_markdownv2_escaping.md`). Ustaw `text_sent_at` na tym samym wpisie signals_log (update, nie nowa linia).
  b. Krok 12 (`active_setups.json` write) następuje po (a), bez zmian.
  c. Card pipeline (krok 13 screenshot → `tv ohlcv` → `tv chart-render` → fill → `tv card render`, z pre-send placeholder check) — wywołaj render z Bash `timeout: 300000` (5 min). Tuż przed renderem: świeży `quote_get`, policz `|current_price − entry_mid| / |tp1 − entry_mid|` względem `decision_at`; jeśli `≥ 0.40` → dopisz `" · ⚠️ rynek już się poruszył od sygnału"` do pola `<SUBTITLE>` przy fill (bez nowego placeholdera — szablon się nie zmienia).
  d. Render sukces → `telegram send-doc` z PNG dokładnie jak krok 14c opisuje, ustaw `card_sent_at`.
  e. Render timeout (Bash zwraca po 300000ms) lub błąd → NIE retry. Zapisz `card_status: "render_failed"` na wpisie signals_log, wyślij `telegram send-text --text "⚠️ Karta niedostępna dla [ID] - zobacz wcześniejszy tekst dla poziomów"` (escaped) na ten sam temat, `card_sent_at` zostaje `null`.

  **Monitoring wdrożenia — pierwsze 5 sygnałów systemowo (2026-07-23):** wdrożone bez wcześniejszego żywego testu na scalp-xau (moduł bez sygnału od 4 dni — czekanie nieuzasadnione). Kompensacja: dla pierwszych **5 sygnałów opublikowanych PO tej zmianie, z DOWOLNEJ z 12 rutyn** (nie tylko scalp-xau), dopisz do `journal/daily_journal.md` szczegółowy log: `id, text_sent_at, card_sent_at, delta_sekundy, fallback_activated (bool), freshness_recheck_activated (bool), subtitle_annotation_correct (bool)`. Licznik w handoffie: `TEXT-FIRST MONITOR n/5`. **Jeśli w tych 5 przypadkach wykryty błąd** (karta nigdy nie dociera mimo udanego render, fallback 14d-e się nie aktywuje mimo timeout/błędu, `text_sent_at`/`card_sent_at` nie są zapisywane) → **ZATRZYMAJ dalsze poleganie na text-first flow, zgłoś jawnie w handoff i daily_journal, nie kontynuuj po cichu z wadliwym mechanizmem.** Po 5 czystych przypadkach ten paragraf monitoringu wygasa — flow działa jako standard bez dodatkowego logowania.
...
17b. **Log phase timings to `journal/daily_journal.md` (added 2026-07-22 — baseline for future time optimization, applies to every session/weekend routine that runs the full 1-18 sequence; scalp-xau routines are exempt, see §7 krok 6.5).** Note `date -u` at the boundary of each phase — before/after Krok 0 macro review, before/after carry-over bar-walk, before/after the watchlist scan, before/after card generation (fill→render→grep), before/after Telegram send — and compute the deltas. Append one line:
`⏱ [routine] [date] [nominal slot]: krok0=Xm | carry_over=Xm | watchlist_scan=Xm | card_gen=Xm | send=Xm | total=Xm`
This is a raw timing ledger, not a narrative — `trw2-daily-close`'s own KROK 4 summary may reference it, but this line itself is appended by every routine directly, independent of daily-close.
18. End.
...
### M5 Scalp XAU — "na dzień dobry" (§7 krok 6.5; od 2026-07-18 WYŁĄCZNIE XAUUSD, równoległe rutyny `trw2-scalp-xau-*`)
...
| P5 | Slot free | No `strategy_type: "scalp_xau"` in `active_setups.json` for this session window |
...
1. **Sweep** of reference range confirmed on M15 or M5 — instrument zawsze XAUUSD (Asia → previous NY H/L; London → Asian H/L; NY → London H/L). No sweep → STOP.
...
4a. **Entry zone = przecięcie OB/FVG M5 z zakresem Fibo 0.5–0.7** (Golden Pocket jako przedział, nie punkt 0.618 — 2026-07-18). Priorytet: FVG pokrywające się z OB na wyższej ramie. Szerszy zakres entry NIE zwalnia z wymogu confluence min. 3 elementów.
5. **TP1 RR ≥ 1.5:1** (obniżone z 2.0, 2026-07-18). Below minimum → STOP, niezależnie od confidence.
6. **Confidence ≥ 60** (ujednolicone z resztą systemu, obniżone z 65 dnia 2026-07-18). Below 60 → STOP. M30 alignment adds +3–5 pts (optional bonus only).
...
### Routing
- Telegram: temat **`swing` — "SWINGI H4/D1"** (`message_thread_id: 9`, chat `-1003969670552`) z tagiem `⚡ SWING RSI-H4` w tytule karty — **NIGDY do `xau` ani innego tematu instrumentu (zmiana 2026-07-18)**. CLI: `--topic swing`.
...
### Routing
- Telegram: **WYŁĄCZNIE temat `swing` — "SWINGI H4/D1"** (`message_thread_id: 9`, chat `-1003969670552`) + tag `📈 SWING` lub `📉 SWING` w tytule — **NIGDY do tematów instrumentów TOP3/XAU (`xau`/`waluty`/`indeksy`/`krypto`) — zmiana 2026-07-18**. CLI: `--topic swing`.
- Lifecycle: `"swing"` w signals_log.jsonl i active_setups.json.
...
**Loss-streak flag (informational only, non-blocking — 2026-07-22, replaces "protection mode"):**

Loss streak is still counted every run (read from `journal/results_log.jsonl`, most recent same-session/same-day entries), and the count is still logged — but it never gates, caps, or degrades publication. Rationale: the old "protection mode" text (4 losses → max 1 new setup/routine, XAU + best-of-day only, no counter-trend) predated the 2026-07-18 3+1 guarantee mechanism and stood in direct, unreconciled conflict with it — a conflict that already surfaced in practice on 2026-07-21 (`journal/daily_journal.md`, "PROTECTION MODE ACTIVATED" entry: the agent suspended the 3+1 guarantee ad hoc, publishing 0/3 TOP3, to honor the older, more restrictive rule). This section resolves that standing conflict in favor of the guarantee: loss history stays visible as context, but is never again allowed to override §7 krok 8c Krok 5's "always 3+1" mandate.

- 2 losses in a row → no change to anything. Continue normal sequence.
- 3 losses in a row, same day → no change to anything. Continue normal sequence.
- 4+ losses in a row, same day → add a visible `⚠️ N strat pod rząd dzisiaj` annotation to the session card and to `handoff_latest.md`, and record `loss_streak_today: N` (number) on every `journal/signals_log.jsonl` entry published that run. That is the **entire** effect.
- Explicitly, the loss-streak flag does **not**: raise the confidence floor, reduce the number of published setups, restrict publication to XAU/best-of-day, or block counter-trend setups. None of those behaviors exist anymore under any name.
- The 3+1 guarantee (§7 krok 8c Krok 5) is untouched by this change — it is a separate mechanism that guarantees signal *count* via confidence degradation, not something the loss-streak flag can override or interact with.
- Terminology: "protection mode" is retired as a name (to avoid any future session pattern-matching on "mode" and reintroducing blocking behavior by association) — use "loss-streak flag" going forward. See `memory/feedback_protection_mode_informational.md` for the full rationale.
...
3. **Dodatkowy warunek natychmiastowy (lekcja sig-089): cena osiągnęła/przekroczyła TP1 przed triggerem → status `missed` od razu przy tym checku** (warunek (c) w §7 krok 2 — tam pełna definicja i format journala: `"TP[n] pre-trigger @ [timestamp baru] — setup missed, no position taken"`), bez czekania na pełną sesję — teza setupu skonsumowana bez pozycji. Priorytet nad kontynuacją pending. (Luka wykryta 2026-07-19 na sig-089, który wisiał PENDING gdy cena minęła TP1 i TP2 bez wejścia.)
...
Jeśli wszystkie trzy prawdziwe → **SOFT FLAG (nie hard block na start):** publikuj z ostrzeżeniem w signals_log: `"flagged_possibly_stale": true` + linia `⚠️ flagged_possibly_stale` w handoff — zamiast automatycznego odrzucenia. **Faza obserwacyjna: zbierz dane z tygodnia** (ile flagowanych setupów faktycznie triggerowało vs zostało missed) przed decyzją o przełączeniu na hard block. **Próg 40% (2026-07-23): wartość startowa, arbitralnie wybrana, do rewizji po zebraniu pierwszych 5–10 realnych przypadków `flagged_possibly_stale` z dowolnej rutyny — nie traktować jako sfinalizowaną stałą dopóki weekly review nie potwierdzi jej trafności.**
...
**Statusy zamknięcia z §9b i §7 krok 2(c) (2026-07-19):** `EXPIRED_NO_ENTRY` (Trigger A — pending nie doszedł do entry przez pełną sesję), **`missed`** (warunek (c) — TP1 osiągnięte pre-trigger, wszystkie strumienie, priorytet nad pending, journal: `"TP[n] pre-trigger @ [timestamp baru] — setup missed, no position taken"`), `SL_HIT` / `TP1_HIT` / `TP2_HIT` (Trigger B — zamknięcie natychmiast przy wykryciu, timestamp z baru dotknięcia). Po każdym z tych statusów (crypto weekend/scalp XAU) w tym samym runie następuje świeży scan i nowa karta (§9b).
...
`id, instrument, group, session_name, signal_date, signal_time_uk, timezone, direction, pattern, setup_type, entry_lower, entry_upper, sl, tp1, tp2, tp3, confidence, rr_planned, risk_pct, htf_alignment, macro_conflict, news_risk, news_context, session_quality, duplicate_check, loss_streak_today, confidence_note, fresh_entry, status, reason_short, decision_at, created_at, published_at, text_sent_at, card_sent_at`

**Field notes (2026-07-22):**
- `loss_streak_today` (number) — replaces the old boolean `protection_mode` field. Count of consecutive same-day losses at publication time, per §9's loss-streak flag — informational only, never gates publication (see §9).
- `decision_at` / `text_sent_at` / `card_sent_at` (ISO timestamps, new 2026-07-22) — close the `created_at`==`published_at` blind spot (historically both were set to the same moment, hiding the real decision→publish timeline). `decision_at` = when the setup cleared the Signal Risk Engine (analysis complete, before card generation). `text_sent_at` = when a stand-alone text alert was sent, if applicable (reserved for a future text-first pilot on scalp-xau, not yet built — `null` on every entry until that pilot ships). `card_sent_at` = when the rendered PNG card was actually sent via `telegram send-doc` (from the CLI's response). `created_at`/`published_at` keep their existing meaning (setup creation / card publication) for backward compatibility with historical rows.
...
**Weekly** (Sunday 20:00): assess confidence-tier performance (70-79 vs 80-89 vs 90+), HTF-aligned vs counter-trend win rate, performance around high-news days, best/worst session quality, whether risk filtering actually screened out bad setups, whether the loss-streak flag (§9, informational only since 2026-07-22) showed up and how it correlated with subsequent results. Record in `weekly_review.md`.
...
- **Trailing SL na runnerze (2026-07-18):** po osiągnięciu TP1 i zabezpieczeniu 70%, SL na pozostałych 30% przesuwa się krokowo w stronę ceny, zachowując stałą odległość: **Swing = 0.2%** od aktualnej ceny, **Scalp = 0.1%** od aktualnej ceny, day trading intraday = 0.2% (jak swing), **Crypto weekend (BTC + watchlista, horyzont ≤44h) = 0.1%** — analogicznie do scalp, ze względu na wyższą zmienność rynku crypto i krótszy horyzont wynikający z expiry §9a (dopisane 2026-07-18 — wcześniej crypto nie miało przypisanej stawki, co wymagało interpretacji przy każdej karcie). SL trailing **nigdy się nie oddala** — przesuwa się wyłącznie w kierunku redukcji ryzyka. Operacyjnie (bar-walk §7 krok 2): przy każdej rutynie/monitoring-checku dla setupu z `tp1_hit: true` przelicz `trailing_sl = max(previous_sl, current_price × (1 − 0.002))` dla LONG (odpowiednio `min(..., × (1 + 0.002))` dla SHORT; 0.001 dla scalpa i crypto weekend) i zapisz w `active_setups.json`. Runner rozliczany na trafieniu trailing SL, TP2/TP3 lub BE — `rr_realized_runner` w results_log liczy się od faktycznego wyjścia. Trailing SL obowiązuje PO TP1 jako uzupełnienie scenariuszy A/B/C (§7b E5) — nie zamiennik: przed TP1 jedynym mechanizmem SL pozostaje reduce-risk-only ze scenariuszy.
...
3. Execution notes
4. Risk notes (Signal Risk Engine: loss-streak flag history — informational only, macro conflicts)
5. Lessons
